...
public partial class TskQuickPane : FluxorComponent {
    ...
    public IEnumerable<DropDown<Project>> ProjectDropDowns { get; set; } = new List<DropDown<Project>>();

    private DropDown<Project> selectedProjectDropDown;

    public List<DropDown<Project>> SelectedProjectDropDown {
        get { return new List<DropDown<Project>> { selectedProjectDropDown }; }
        set {
            selectedProjectDropDown = value.First();
    ...
    public IEnumerable<DropDown<Initiative>> InitiativeDropDowns { get; set; } = new List<DropDown<Initiative>>();
    ...
    private void UpdateProjectDropDown() {
        if (ProjectsState.HasValue()) {
            ProjectDropDowns = ProjectsState.Value.Entities.OrderBy(p => p.Name)
                                                   .Select(p => new DropDown<Project>(p))
                                                   .ToList();
            ...
            if (selectedProjectDropDown == null) {
                ...
                if (genericProject != null)
                    selectedProjectDropDown = new DropDown<Project>(genericProject);
            }
    ...
    private void UpdateInitiativeDropDown() {
        ...
        if (InitiativesState.HasValue()) {
            ...
            var selectedProjectId = selectedProjectDropDown.Entity.Id;
            ...
            var index = 1;
            InitiativeDropDowns = InitiativesState.Value.Entities.Where(i => i.ProjectId == selectedProjectId)
                                                                 .OrderBy(i => i.Name)
                                                                 .Select(i => new DropDown<Initiative>(i))
                                                                 .ToList();
        ...
        try {
            ...
            if (!string.IsNullOrEmpty(NewTskName) && !IsInInitiativeCreationMode) {
                ...
                await EntityStateFacade.Add<Initiative, InitiativeAdd.Command>(new InitiativeAdd.Command {
                    ...
                    ProjectId = selectedInitiativeId
                });
